...
#figure(
  image("../res/file_server_dd.png", width: 90%),
...
#figure(
  image("../res/fdisk_server_dd.png", width: 90%),
  caption: [Ausgabe von `fdisk -l Server.dd`],
) <fig-fdisk>

#figure(
  image("../res/mmls_server_dd.png", width: 90%),
...
#figure(
  image("../res/fsstat_not.png", width: 90%),
...
#figure(
  image("../res/losetup_Server_dd.png", width: 90%),
...
#figure(
  image("../res/lsblk_blkid_Server_dd.png", width: 90%),
...
#figure(
  image("../res/Namenskonflikt_gelöst.png", width: 90%),
...
#figure(
  image("../res/Aktivierung_Asservat.png", width: 90%),
...
#figure(
  image("../res/mount_asservat.png", width: 90%),
...
#figure(
  image("../res/Beweis1_server_dd.png", width: 90%),
...
#figure(
  image("../res/Beweis2_bash_history_server_dd.png", width: 90%),
...
#figure(
  image("../res/Beweis4_Löschbefehle_server_dd.png", width: 90%),
...
#figure(
  image("../res/Beweis3_Projektdaten_server_dd.png", width: 90%),
...
#figure(
  image("../res/Beweis5_Zustand_Projektverzeichnis_server_dd.png", width: 70%),
...
#figure(
  image("../res/Beweis7_manipulierte_daten_server_dd.png", width: 90%),
...
#figure(
  image("../res/Beweis6_Timestomping_kundencsv_server_dd.png", width: 90%),
...
#figure(
  image("../res/banner_server_ram.png", width: 90%),
...
#figure(
  image("../res/pslist_server_ram.png", width: 90%),
...
+ *Netzwerkmitschnitt referenziert*: Aus der rekonstruierten Bash-History sowie aus `auth.log` geht hervor, dass durch die Session svc ein vollständiger Netzwerkmitschnitt (`/tmp/silent_quarry.pcap`) erzeugt wurde. Eine Aussage über dessen Status im teamweiten Asservaten-Schema lässt sich aus den Asservaten 01/03 allein nicht treffen; eine Abstimmung mit der Netzwerkforensik wird empfohlen (B010-RAM-SQ-2026).

== Gesamtfazit (M2 – Server und RAM)

Die kombinierte Analyse von Datenträger- und Arbeitsspeicherabbild des Linux-Servers zeichnet ein konsistentes, durch zwei unabhängige Quellen bestätigtes Bild: 

Der Benutzer m.vogel griff am 05.07.2026 zweimal per SSH von 192.168.50.10 auf das System zu (B003-SERVER-SQ-2026) und löschte anschließend gezielt die Verzeichnisse Projekte und Kunden (B004-SERVER-SQ-2026, B006-RAM-SQ-2026), wobei die Zeitstempel manipuliert wurden, um die Aktivität zu verschleiern (B005-SERVER-SQ-2026, B008-SERVER-SQ-2026). 

Die davon unabhängige, zeitlich überlappende Konsolensitzung des Systemkontos svc (B004-RAM-SQ-2026, B005-RAM-SQ-2026) steht im Zusammenhang mit der Vorbereitung des Testszenarios sowie der ordnungsgemäßen forensischen Sicherung (Netzwerkmitschnitt, RAM-Dump) und ist klar vom eigentlichen Vorfall abzugrenzen. 

Für die weitere teamübergreifende Analyse wird empfohlen, die in der Bash-History nachgewiesene Erzeugung des Netzwerkmitschnitts `silent_quarry.pcap` (B010-RAM-SQ-2026) mit der Netzwerkforensik abzustimmen, um dessen Auswertung sicherzustellen.