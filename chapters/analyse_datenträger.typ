...
Unter Projektverzeichnis Kunden wurde eine Datei `kunden_2026.csv` gefunden. Unter dem Verzeichnis `Kunden` wurde die Datei `kunden_2026.csv` gefunden. Die Inode-Nummer `263241` wurde mit `fls` identifiziert und anschließend mit `icat` zur Extraktion der Datei verwendet.
...
In diesem Verzeichnis war jedoch nur `desktop.ini` sichtbar. Es wurden keine `$I`- oder `$R`-Dateien gefunden. Zusätzlich wurde `Rifiuti2` gegen das Benutzerverzeichnis im Papierkorb auch ausgeführt. Das Tool konnte den Pfad öffnen, meldete jedoch einen leeren Ordner und gab keine gelöschten Dateien aus.

#align(center)[
  #image("../res/Images_M4/clean_rifiuti.png", width: 90%)
]

Damit konnte aus `Client.dd` kein rekonstruierbarer Papierkorb-Löschvorgang nachgewiesen werden. Dieses Ergebnis widerspricht dem Angriffsszenario jedoch nicht zwangsläufig, da die relevanten Löschhandlungen laut Serverartefakten auf dem Server stattfanden.Das Ergebnis bedeutet lediglich, dass im untersuchten Client-Abbild keine auswertbaren Papierkorb-Artefakte in Form von `$I`- oder `$R`-Dateien vorhanden waren.
...
#table(
  ...
  [`2026-07-04 21:08:31`], [Metadatenänderung am Verzeichnis `/srv/projekte/Verwaltung`], [`istat`, `mactime`],
